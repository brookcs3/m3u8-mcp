...
pub fn run() {
    ...
    // Clone state for the setup hook
    ...
    tauri::Builder::default()
        ...
        .setup(move |_app| {
            ...
            let server_handle = server_handle_clone.clone();
            tauri::async_runtime::spawn(async move {
                let default_port: u16 = 3333;
                let default_tools = vec![
                    ...
                    "m3u8_set_url".to_string(),
                    "m3u8_get_url".to_string(),
                ];
                
                let handle = server_handle.lock().await;
                let new_state = Arc::new(mcp_server::McpServerState::new_with_tools(default_port, default_tools));
                ...
                *state_lock = Some(new_state.clone());
                ...
                let mut port_lock = handle.port.lock().await;
                *port_lock = Some(default_port);
                drop(port_lock);
                ...
                let task_handle = tokio::spawn(async move {
                    if let Err(e) = mcp_server::start_mcp_server(task_state.clone()).await {
                        eprintln!("MCP Server auto-start error: {}", e);
                        *task_state.running.lock().await = false;
                    }
                });
                
                let mut stored_handle = handle.handle.lock().await;
                *stored_handle = Some(task_handle);
                
                println!("MCP Server auto-started on port {}", default_port);
            });
            Ok(())
        })
